...
export function TechMarquee() {
  ...
  return (
    <div className="relative z-30 -mx-4 mb-0 mt-0 overflow-hidden border-y border-[#1f2428]/10 bg-[#f2dec7]/92 px-7 py-4 text-[#1f2428] shadow-[0_-18px_42px_rgba(107,109,67,.10),0_18px_42px_rgba(107,109,67,.12)] backdrop-blur-md sm:-mx-8">
      ...
      <motion.div className="relative flex w-max gap-3 sm:gap-4" animate={{ x: ["0%", "-50%"] }} transition={{ duration: 28, repeat: Infinity, ease: "linear" }}>
        {items.map(([name, , color, type, iconColor], index) => (
          <div key={`${name}-${index}`} className="group flex h-11 items-center gap-2.5 rounded-full border border-[#1f2428]/12 bg-[#f7eadb]/82 py-1 pl-1.5 pr-4 text-xs font-black tracking-[0.02em] text-[#1f2428]/86 shadow-[0_8px_22px_rgba(107,109,67,.13)] backdrop-blur transition hover:-translate-y-0.5 hover:bg-[#f2dec7] sm:h-12 sm:gap-3 sm:pr-5 sm:text-sm">
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-[#1f2428] ring-1 ring-[#1f2428]/12 transition group-hover:scale-105 sm:h-9 sm:w-9" style={{ backgroundColor: color }}>
...
export function SkillBadge({ item, index }: { item: StackItem; index: number }) {
  ...
  return (
    <motion.article
      ...
      whileTap={{ scale: 0.97 }}
      className="group relative min-h-56 overflow-hidden rounded-xl border-2 border-[#151515] bg-white p-5 shadow-[6px_6px_0_#151515]"
    >
      <motion.div
      ...
      <h3 className="relative text-3xl font-black tracking-[-0.04em]">{name}</h3>
